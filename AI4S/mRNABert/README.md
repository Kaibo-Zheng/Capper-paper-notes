...
这篇文章想解决的核心问题是：现有 RNA/mRNA 模型大多只擅长局部区域建模，比如只看 5'UTR、只看 CDS 或只看 3'UTR，但真实的 mRNA 设计任务需要把完整序列作为一个整体来理解和优化。

一句话总结：mRNABERT 试图把完整 mRNA 当成统一对象来学习，并进一步借助蛋白语言模型，把核酸序列和蛋白语义对齐，最终服务于更真实的全长 mRNA 设计任务。

我读完整篇后的判断是，这篇论文最有价值的地方不只是“又做了一个 RNA 大模型”，而是把全长 mRNA 建模、跨区域统一表示和蛋白语义对齐三件事真正拼在了一起。

## Method

作者先整理出约 1800 万条高质量、非冗余 mRNA 序列作为预训练语料，然后在此基础上做了三层关键设计：

1. **双重分词**：UTR 按单核苷酸切分，CDS 按密码子切分，同时保住 UTR 的单碱基分辨率和 CDS 的翻译语义。
2. **长序列建模**：模型主体是 12 层、hidden size 768 的 BERT 编码器，用 ALiBi 代替传统位置编码，并引入 Flash Attention 提高长序列训练效率。
3. **跨模态对比学习**：在 MLM 预训练后，再拿 50 万条 CDS 及其翻译后的蛋白序列，和冻结的 ProtT5-XL 表征做对比学习，把核酸和蛋白语义投到同一个空间。
...
- **模型层面**：主体是 12 层 Transformer encoder。训练阶段先做 MLM，mask ratio 为 15%，训练长度默认限制在 1022 tokens。
- **跨模态层面**：第二阶段选取 50 万条 CDS，把翻译后的氨基酸序列输入冻结的 ProtT5-XL，分别得到核酸和蛋白 embedding，再投影到 256 维共享空间做对比学习。
- **应用层面**：作者把模型统一用于 5'UTR、CDS、3'UTR、蛋白属性和全长 mRNA 属性预测，目标是构建一个真正的 all-in-one mRNA foundation model。
...
- 这张图回答的是一个关键问题：模型到底有没有学到生物学语义，而不是只记住 token 的共现频率。
- **A/B 对比 C/D** 体现了对比学习的作用。没有 contrastive learning 时，同义密码子和氨基酸性质的聚类都比较散；加入后，同义密码子明显靠拢。
- 作者给出了量化指标：按氨基酸性质聚类时，ARI 从 **0.166 提升到 0.498**，FMI 从 **0.325 提升到 0.596**。这说明蛋白语义注入后，latent space 的结构明显更合理。
- **E 图** 说明模型不仅能区分 CDS、5'UTR、3'UTR，还能把 lncRNA 和 mRNA 分开，说明它学到的不是简单的区域模式匹配。
- **F 图** 则显示同源序列会按物种自然聚类，说明 embedding 里保留了跨物种的进化信息。

### 图 3：5'UTR 考试成绩（翻译起始效率）

![Fig. 3](./fig3.png)

- 这里评估的是 8 个 5'UTR MPRA 数据集上的平均核糖体负载（MRL）预测，指标是 Spearman 相关系数。
- mRNABERT 在最大的两个随机 UTR 数据集 **U1 / U2** 上拿到了 **0.962** 和 **0.924**，都是该任务上的最强结果。
- 在剩余 6 个数据集里，mRNABERT 还拿下了 3 个第一。总体上，它和专门为 5'UTR 设计的 UTR-LM 基本打平，都是 **8 个任务里 4 个第一**。
- 这说明 mRNABERT 不是靠牺牲局部任务性能来换“全能”；在 5'UTR 这个强基线很多的场景里，它仍然能达到顶尖水平。
...
- **m6A 位点预测（B）**：原文写得比较克制。mRNABERT 在 9 个细胞系里表现 **稳定第二**，紧跟 3UTRBERT，但超过了其他方法。
- 这张图真正传达的信息不是“mRNABERT 在 3'UTR 所有任务上绝对第一”，而是：即使它不是专门只在 3'UTR 上预训练的模型，也能在转录后调控任务上逼近甚至部分超过专用模型。

### 图 5：跨模态降维打击（蛋白质属性预测）

![Fig. 5](./fig5.png)

- 这一节是整篇文章最有意思的地方之一：作者直接拿 mRNA 模型去做蛋白相关任务，检验跨模态对比学习是否真的带来可迁移语义。
- **蛋白熔点预测（A）**：加入对比学习后，mRNABERT 的 R² 从 **0.60 提升到 0.77**，只略低于 CaLM 的 **0.78**，但已经超过不少更大的纯蛋白模型，文中点名 ProtT5-XL 最好也只有 **0.73**。
- **蛋白溶解度预测（B）**：mRNABERT 达到 **R² = 0.63**，超过 CaLM 的 **0.61**，也明显优于没有做对比学习的版本，只略落后于最大的一些蛋白模型。
- **7 个物种的转录本丰度预测（C）**：mRNABERT 在 7 个物种里有 5 个优于 CaLM。以 **Homo sapiens** 为例，mRNABERT 的 R² 为 **0.38**，高于 CaLM 的 **0.35**。
- 这张图说明的不是“mRNA 模型能替代蛋白模型”，而是：把编码序列和蛋白序列对齐之后，mRNA embedding 的信息密度显著提高，足以外溢到部分蛋白工程任务。
