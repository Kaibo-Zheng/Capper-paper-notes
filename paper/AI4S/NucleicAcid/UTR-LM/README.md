...
所以 Fig. 6 的可解释性分析不是单纯看一张热图，而是有一套明确的片段打分流程。
...
- Fig. 2 先做模型变体和训练策略消融，再比较不同 predictor 与不同 baseline，
  说明收益主要来自预训练表示和全模型微调，而不是某个复杂下游头偶然带来的提升。
- 论文给出的代表性结论是：
...
- Fig. 3 的重点是内源数据，而不是 50 bp 随机库；
  这里的结果更能说明模型是否真的可用于真实转录本环境。
- 文中给出的总结是：
